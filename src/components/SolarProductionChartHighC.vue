...
let pendingData: [number, number][] | null = null
...
function yExtentWithPadding(data: [number, number][]) {
  if (!data.length) return null
  let min = data[0][1]
  let max = data[0][1]
...
function updateSeries(data: [number, number][]) {
  if (!chart) return
...
function createChart() {
  ...
  // subtitle: { text: subtitleText.value, align: 'left' },
  subtitle: { text: undefined },
    ...
    series: [{ type: 'column', data: seriesData.value, name: 'Production', color: '#ffc107', borderWidth: 0, pointPadding: 0.05 }]
