...
function getAggregationMode(): AggregationMode {
  ...
  // Dashboard offers 1m and 1y durations; year view should be monthly.
...
function buildAggregationCacheKey(mode: AggregationMode, dataPoints: typeof points.value) {
  if (!dataPoints.length) return `daily-consumption|${mode}|empty`
  ...
  return `daily-consumption|${mode}|${startMs}|${endMs}|${dataPoints.length}|${first}|${last}`
}
...
function aggregateForRange(dataPoints: typeof points.value): AggregatedData[] {
  ...
  const bucketMap = new Map<number, { selfConsumption: number; imported: number }>()

  for (const point of dataPoints) {
    const pointMs = Date.parse(point.time)
    if (!Number.isFinite(pointMs)) continue
    const dayStartMs = startOfLocalDay(pointMs)
      ...
      mode === 'year'
        ...
        : mode === 'month'
          ...
          : mode === 'hour'
            ...
            : dayStartMs
    const existing = bucketMap.get(bucketMs) || { selfConsumption: 0, imported: 0 }

    // total consumption is built from:
    // imported = in (purchased from grid)
    // selfConsumption = self_cons (solar consumed directly)
    existing.selfConsumption += point.self_cons || 0
    existing.imported += point.in || 0
  ...
  if (startMs == null || endMs == null) {
    const result = Array.from(bucketMap.entries())
      ...
      .map(([bucketStartMs, data]) => ({
        ...
        imported: Number(data.imported.toFixed(2))
      }))
  ...
  if (mode === 'hour') {
    ...
    while (endExclusiveMs != null && cursor < endExclusiveMs) {
      const data = bucketMap.get(cursor) || { selfConsumption: 0, imported: 0 }
      rows.push({
...
function updateChartData() {
  ...
  const importedData = aggregatedData.map((d) => d.imported)
  const selfConsumptionData = aggregatedData.map((d) => d.selfConsumption)

  chart.xAxis[0].setCategories(categories, false)
  chart.series[0].setData(importedData, false)
  ...
  chart = Highcharts.chart(chartRef.value, {
    ...
    tooltip: {
      ...
      formatter: function () {
        ...
        s += `<b>Total: ${total.toFixed(2)} kWh</b>`
        return s
      }
    },
